                ...
                @forelse($equipo->hojaVida as $h)
                    @php
                        $colores = [
                            ...
                            'daño_fisico' => 'badge-dark',
                            'robo_perdida' => 'badge-danger',
                            'incidente' => 'badge-danger',
                            'otro' => 'badge-light',
                        ];

                        $color = $colores[$h->evento] ?? 'badge-info';
                    @endphp

                    <tr>
                        <td>{{ $h->created_at->format('Y-m-d H:i') }}</td>
                        <td><span class="badge {{ $color }}">{{ strtoupper($h->evento) }}</span></td>
                        <td>{{ $h->descripcion ?? 'Sin detalles' }}</td>
