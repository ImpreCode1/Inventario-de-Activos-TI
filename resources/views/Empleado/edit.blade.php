                    ...
                    <form action="/empleados/{{ $empleado->id }}" method="POST">
                        ...
                        <div>
                            ...
                            <input type="text" name="nombre" id="nombre" class="form-control"
                                value="{{ $empleado->nombre ?? 'No existe' }}" placeholder="Nombre" tabindex="1">
                        </div>
                        <div>
                            ...
                            <select name="id_cargo" id="id_cargo" class="form-control" tabindex="2">
                                ...
                                @foreach ($cargo as $cargo)
                                    <option value="{{ $cargo->id }}" @if(old('id_cargo', $empleado->id_cargo ?? 'No existe') == $cargo->id) selected @endif>{{ $cargo->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="id_depto" id="id_depto" class="form-control" tabindex="3">
                                ...
                                @foreach ($departamento as $depto)
                                    <option value="{{ $depto->id }}" @if(old('id_depto', $empleado->id_depto ?? 'No existe') == $depto->id) selected @endif>{{ $depto->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="usu_dominio" id="usu_dominio" class="form-control"
                                value="{{ $empleado->usu_dominio ?? 'No existe' }}" placeholder="Usuario de dominio"
                                tabindex="7">
                        </div>
                        <div>
                            <label for="" class="form-label">Clave de dominio</label>
                            <input type="text" name="clave_dominio" id="clave_dominio" class="form-control"
                                value="{{ $empleado->clave_dominio ?? 'No existe' }}" placeholder="Clave de dominio"
                                tabindex="8">
                        </div>
                        <div>
                            <label for="" class="form-label">Email</label>
                            <input type="text" name="email" id="email" class="form-control"
                                value="{{ $empleado->email ?? 'No existe' }}" placeholder="Email" tabindex="9">
                        </div>
                        <div>
                            <div>
                                ...
                                <select name="id_modo_usuario" id="id_modo_usuario" class="form-control" tabindex="12"
                                    value="{{ $empleado->id_modo_usuario ?? 'No existe' }}">
                                    <option value="">-- Modo Usuario del empleado --</option>
                                    @foreach ($modoUsuario as $modoUsuario)
                                    <option value="{{ $modoUsuario->id }}" @if(old('id_cargo', $empleado->id_modo_usuario ?? 'No existe') == $modoUsuario->id) selected @endif>{{ $modoUsuario->cargo }}</option>
                                @endforeach
                                </select>
                            </div>
                            <br>
